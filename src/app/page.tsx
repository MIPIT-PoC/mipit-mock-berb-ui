...
const REJECT_CODES_BREB = ['BREB001', 'BREB002', 'BREB003'];
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    QUEUED:    'bg-amber-500/20 text-amber-400 ring-1 ring-amber-500/30',
    ROUTED:    'bg-sky-500/20 text-sky-400 ring-1 ring-sky-500/30',
...
function FlowDiagram({ dest, status }: { dest: DestRail; status?: string }) {
  ...
  const line = done ? 'bg-emerald-500' : failed ? 'bg-red-500' : 'bg-amber-500';
  const hub  = done ? 'bg-emerald-500 shadow-emerald-500/30' : failed ? 'bg-red-500 shadow-red-500/30' : 'bg-amber-500 shadow-amber-500/30 animate-pulse';
  return (
    <div className="glass-card rounded-2xl p-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-col items-center gap-1 min-w-[60px]">
          ...
          <span className="text-xs font-bold text-zinc-200">BRE_B</span>
          <span className="text-[10px] text-zinc-500">Colombia</span>
        </div>
      ...
      {PAYMENT_STAGES.map((stage) => {
        ...
        return (
          <div key={stage.key} className="flex items-center gap-3">
            <div className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center transition-colors duration-500 ${ts ? 'bg-amber-500' : 'bg-zinc-700'}`}>
            ...
            {ts && <span className="text-xs font-mono text-zinc-500">{new Date(ts).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>}
          </div>
        );
      })}
    </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="px-5 py-4 flex items-center justify-between border-b border-zinc-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-zinc-100 text-sm">BRE_B (Colombia)</h3>
              <p className="text-xs text-zinc-500">Adaptador mock - puerto 9103</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="p-5 space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setConfig((c) => ({ ...c, enabled: !c.enabled }))}
              className={`flex items-center gap-1.5 transition-colors ${config.enabled ? 'text-amber-500' : 'text-zinc-600'}`}>
              {config.enabled ? <ToggleRight className="w-8 h-8 text-amber-500" /> : <ToggleLeft className="w-8 h-8 text-zinc-600" />}
            </button>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-amber-500">{config.rejectionRate}%</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={config.minLatencyMs}
                ...
                className="w-full px-3 py-2 text-sm bg-zinc-800/80 border border-zinc-700 rounded-lg outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 text-zinc-200" />
            </div>
            <div>
          ...
          <button onClick={saveConfig} disabled={actionLoading === 'config'}
            className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-zinc-900 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors text-sm">
...
export default function BrebSimulatorPage() {
  ...
  const intlForm = useForm<IntlValues>({
    ...
    defaultValues: {
      ...
      amount: 250000.00, reference: 'BREB-INTL-001',
    },
  });
  ...
  const onIntlSubmit = async (data: IntlValues) => {
    try {
      ...
      const body = {
        ...
        purpose: 'TRANSFERENCIA', reference: data.reference || 'BREB-INTL',
      };
  ...
  const inp = 'w-full px-3 py-2 text-sm bg-zinc-800/80 border border-zinc-700 rounded-lg outline-none transition focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 text-zinc-200 placeholder:text-zinc-500';
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-10 glass border-b border-zinc-800/50">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg flex items-center justify-center text-lg shadow-lg shadow-amber-500/20">🇨🇴</div>
            <div>
              <span className="text-sm font-bold text-zinc-100">BRE_B Simulator</span>
              <span className="ml-2 text-xs text-zinc-500">Banco de la Republica - Colombia</span>
            </div>
          </div>
        ...
        <div className="inline-flex glass-card rounded-xl p-1 mb-8">
          {([
            ...
            <button key={id} onClick={() => setMode(id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${mode === id ? 'bg-amber-500 text-zinc-900 shadow-lg shadow-amber-500/20' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
              <Icon className="w-4 h-4" />{label}
            </button>
          ))}
        </div>

        {mode === 'simulator' && <BankingSimulator adapterUrl={adapterUrl} rejectCodes={REJECT_CODES_BREB} />}
              ...
              {mode === 'local' ? (
                <>
                  <div className="px-6 py-4 bg-gradient-to-r from-amber-500 to-amber-600">
                    <h2 className="text-base font-semibold text-zinc-900">Simular Transaccion BRE_B</h2>
                    <p className="text-xs text-amber-900/70 mt-0.5">Flujo bancario local dentro de Colombia</p>
                  </div>
                  ...
                  <div className="px-5 pt-4">
                    <div className="text-xs px-3 py-2 rounded-md border border-amber-500/40 bg-amber-500/10 text-amber-200">
                      <strong>Modo Local — demo shortcut.</strong> Este endpoint <code>/api/simulate/breb</code> NO pasa
                  ...
                  <form onSubmit={localForm.handleSubmit(onLocalSubmit)} className="p-5 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Identificador Ordenante</label>
                        <input {...localForm.register('debtorAlias')} placeholder="BREB-+573005551234" className={`${inp} font-mono`} />
                        <p className="text-[10px] text-zinc-500 mt-0.5">Telefono, NIT o Email</p>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Nombre Ordenante</label>
                        <input {...localForm.register('debtorName')} placeholder="Carlos Rodriguez" className={inp} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                      ...
                      <div>
                        ...
                        <input {...localForm.register('creditorName')} placeholder="Ana Lopez" className={inp} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input {...localForm.register('reference')} placeholder="BREB-TEST-001" className={inp} />
                      </div>
                    </div>
                    <button type="submit" disabled={localStatus === 'loading'}
                      className="w-full mt-1 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-zinc-900 font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors text-sm shadow-lg shadow-amber-500/20">
                      {localStatus === 'loading' ? <><Loader2 className="w-4 h-4 animate-spin" />Simulando...</> : <><Send className="w-4 h-4" />Simular Transaccion BRE_B</>}
                    </button>
                  </form>
                </>
              ) : (
                <>
                  <div className="px-6 py-4 bg-gradient-to-r from-sky-500 to-blue-600">
                    <h2 className="text-base font-semibold text-white">Pago Internacional BRE_B</h2>
                    <p className="text-xs text-sky-200/70 mt-0.5">Flujo completo a traves de mipit-core</p>
                  </div>
                  <form onSubmit={intlForm.handleSubmit(onIntlSubmit)} className="p-5 space-y-4">
                    <div className="flex items-center gap-2.5 px-3 py-2.5 bg-amber-500/10 rounded-xl border border-amber-500/20">
                      ...
                      <div>
                        <div className="text-[10px] font-semibold text-amber-500 uppercase tracking-wide">Rail Origen</div>
                        <div className="text-sm font-bold text-amber-400">BRE_B - Colombia - COP</div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Identificador Ordenante</label>
                        <input {...intlForm.register('debtorAlias')} placeholder="BREB-+573005551234" className={`${inp} font-mono`} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Nombre Ordenante</label>
                        <input {...intlForm.register('debtorName')} placeholder="Carlos Rodriguez" className={inp} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 mt-0.5">{destRail === 'PIX' ? 'PIX-{llave PIX}' : 'SPEI-{CLABE 18}'}</p>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Nombre Beneficiario</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-1">Referencia</label>
                        <input {...intlForm.register('reference')} placeholder="BREB-INTL-001" className={inp} />
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              {mode === 'local' && localStatus === 'idle' && (
                <div className="glass-card rounded-2xl p-10 flex flex-col items-center justify-center min-h-72">
                  ...
                  <p className="text-sm text-zinc-500 text-center">Completa el formulario para simular una transaccion BRE_B</p>
                </div>
              )}
              {mode === 'local' && localStatus === 'loading' && (
                <div className="glass-card rounded-2xl p-10 flex flex-col items-center justify-center min-h-72">
                  <Loader2 className="w-10 h-10 text-amber-500 animate-spin mb-4" />
                  <p className="text-sm text-zinc-400 font-medium">Simulando flujo bancario BRE_B...</p>
                </div>
              )}
              {mode === 'local' && localStatus === 'success' && localResponse && (
                <div className="glass-card rounded-2xl overflow-hidden">
                  <div className="flex items-center gap-2 px-5 py-3.5 bg-emerald-500/10 border-b border-emerald-500/20">
                    <CheckCircle className="w-4 h-4 text-emerald-400" /><span className="text-sm font-semibold text-emerald-400">Transaccion Simulada</span>
                  </div>
                  <div className="p-5">
              ...
              {mode === 'international' && intlStatus === 'idle' && (
                <div className="glass-card rounded-2xl p-10 flex flex-col items-center justify-center min-h-72">
                  ...
                  <p className="text-sm text-zinc-500 text-center mb-4">Envia un pago desde BRE_B hacia otro riel internacional</p>
                  <div className="flex items-center gap-2 text-xs text-zinc-500">
                    <span>🇨🇴 BRE_B</span><ArrowRight className="w-3 h-3" /><span className="text-zinc-600">mipit-core</span><ArrowRight className="w-3 h-3" /><span>🇧🇷 PIX - 🇲🇽 SPEI</span>
                  </div>
                </div>
              )}
                  ...
                  <div className="glass-card rounded-2xl overflow-hidden">
                    <div className="px-5 py-3.5 border-b border-zinc-700/50 flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                          <span>🇨🇴 BRE_B</span><ArrowRight className="w-3 h-3" />
                          <span>{RAIL_INFO[destRail].flag} {RAIL_INFO[destRail].name}</span>
                          <span className="ml-1 font-semibold text-zinc-300">{payment.amount?.toLocaleString('es-CO', { style: 'currency', currency: payment.currency || 'COP' })}</span>
                        </div>
                      </div>
                      {intlStatus === 'polling' && <RefreshCw className="w-4 h-4 text-amber-400 animate-spin flex-shrink-0" />}
