...
const brebFormSchema = z.object({
  debtorAlias: z.string().min(1, 'Identificador del ordenante requerido'),
  debtorName: z.string().optional(),
  creditorAlias: z.string().min(1, 'Identificador del beneficiario requerido'),
...
type BrebFormValues = z.infer<typeof brebFormSchema>;
...
export default function BrebSimulatorPage() {
  ...
  } = useForm<BrebFormValues>({
    resolver: zodResolver(brebFormSchema),
    defaultValues: {
      debtorAlias: '+573005551234',
      debtorName: 'Carlos Rodriguez',
      creditorAlias: '1234567890-1',
      creditorName: 'Ana López',
      amount: 250000.0,
      currency: 'COP',
      purpose: 'TRANSFERENCIA',
      reference: 'BREB-TEST-001',
    },
  });

  const onSubmit = async (data: BrebFormValues) => {
    try {
      setStatus('loading');
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
      
      const response = await fetch(`${apiUrl}/api/simulate/breb`, {
        method: 'POST',
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-gray-900">BRE_B Simulator</h1>
          </div>
          <p className="text-gray-600">Simula transacciones BRE_B en tiempo real</p>
        </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="+573005551234 o 1234567890-1"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
                <p className="text-xs text-gray-500 mt-1">Teléfono, NIT o Email</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="+573115552345 o 9876543210-1"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-semibold py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
              >
